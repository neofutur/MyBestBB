...
if (isset($_POST['gen_news']))
{
	...
	$result = $db->query('SELECT id, subject FROM '.$db->prefix.'topics WHERE forum_id='.$forum_id.' ORDER BY sticky DESC, posted DESC LIMIT 0, '.$num_posts_index) or error('Impossible de récupérer la liste des sujets', __FILE__, __LINE__, $db->error());
	if (!$db->num_rows($result))
		message('Il n\'y a pas de sujets pour générer les news à partir du forum ID = '.$forum_id.'.');

	require PUN_ROOT.'include/parser.php';

	$news_tpl = file_get_contents($template_path) or error('Impossible d\'ouvrir le fichier template '.$template_path.'. Assurez-vous que la variable $template_path est correctement renseignée.', __FILE__, __LINE__);

	$fh = @fopen($output_dir_latest.'news.html', 'wb');
	if (!$fh)
		error('Impossible d\'écrire les news dans '.$output_dir_latest.'news.html. Veuillez vous assurer que PHP possède les droits d\'écriture dans le répertoire '.$output_dir_latest, __FILE__, __LINE__);
	...
	$result = $db->query('SELECT MIN(posted) FROM '.$db->prefix.'topics WHERE forum_id='.$forum_id.'') or error('Impossible de récupérer les sujets les plus récents', __FILE__, __LINE__, $db->error());
	...
	{
		$result = $db->query('SELECT id, subject FROM '.$db->prefix.'topics WHERE forum_id='.$forum_id.' AND posted>=UNIX_TIMESTAMP(\''.$year_start.'-'.$month_start.'-01\') AND posted<UNIX_TIMESTAMP(\''.$year_end.'-'.$month_end.'-01\') ORDER BY posted DESC') or error('Impossible de récupérer la liste des sujets', __FILE__, __LINE__, $db->error());
		if ($db->num_rows($result))
		{
			$news_tpl = file_get_contents($template_path) or error('Impossible d\'ouvrir le fichier template '.$template_path.'. Assurez-vous que la variable $template_path est correctement renseignée.', __FILE__, __LINE__);
			...
			if (!$fh)
				error('Impossible d\'écrire les archives des news dans '.$output_dir_archive.$year_start.'-'.($month_start > 9 ? $month_start : '0'.$month_start).'.html. Veuillez vous assurer que PHP possède les droits d\'écriture dans le répertoire '.$output_dir_archive, __FILE__, __LINE__);
...
?>
	<div class="block">
		<h2><span>Résultats du Générateur de News</span></h2>
		<div class="box">
			<div class="inbox">
				<p>News générées avec succès.</p>
			</div>
		</div>
	</div>
<?php
...
?>
	<div class="blockform">
		<h2><span>Générateur de News</span></h2>
		<div class="box">
			<form id="example" method="post" action="<?php echo $_SERVER['REQUEST_URI'] ?>&amp;foo=bar">
				<div class="inform">
					<fieldset>
						<legend>Générer un contenu de news statique</legend>
						<div class="infldset">
							<p>Ce plugin récupère les messages depuis le forum spécifié et génére un contenu statique de ces messages sur le modèle d'un template de news. Il créer un fichier news.html qui contient les plus récentes news et une archive mensuelle  (ex. 2005-01.html) pour tous les messages du forum. <strong>Regardez en haut du fichier source  PHP le code pour ce plugin afin de changer les paramètres que vous voyez ci-dessous.</strong></p>
							<table class="aligntop" cellspacing="0">
								<tr>
									<th scope="row">Récupère les messages du forum</th>
								...
								<tr>
									<th scope="row">Enregistre les dernières news dans</th>
							...
							<div class="fsetsubmit"><input type="submit" name="gen_news" value="Générer les News" tabindex="1" />
							</div>
						</div>
					</fieldset>
				</div>
