<?php
	...
	// Fetch user info	
	$res = $fdb->query('SELECT e.*,m.* FROM '.$_SESSION['php'].'members AS m LEFT JOIN '.$_SESSION['php'].'member_extra AS e ON m.id=e.id WHERE m.id>'.$start.' ORDER BY m.id LIMIT '.ceil($_SESSION['limit']/5)) or myerror('Unable to fetch user info', __FILE__, __LINE__, $fdb->error());
	$last_id = 0;
