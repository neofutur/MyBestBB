...
if (isset($_POST['assign_award']))
{
	...
	$query='UPDATE '.$db->prefix.'users SET imgaward="'.$db->escape($_POST['award_filename']).'" WHERE id="'.intval($_POST['award_user_id']).'" LIMIT 1';

	$db->query($query) or error('Error when assigning new image award to user.',__FILE__,__LINE__,$db->error());
$answer="<b>Award ".$_POST['award_filename']."  : "."<img src='/img/awards/".$_POST['award_filename']."' alt='' />"." added to user ".intval($_POST['award_user_id'])." !</b>";
//	message('Award added.');
//exit;		
}
...
?>
	<div id="exampleplugin" class="blockform">
		<?php if ( $answer != '' ){ ?>
		<h2><span><?php echo $answer ?></span></h2> <?php } ?>
...
<?php // Note that the script just ends here. The footer will be included by admin_loader.php. ?>
