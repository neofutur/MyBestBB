...
et ensuite accedez a la page AP_SMilies.php via la section d'aministration de punbb. Il n'y a rien a valider, 
il suffit d'afficher la page.

Des lors, si vous entrez :rigolade: dans un post, le code sera remplace par le smiley.

les caracteres autorise sont toutes les lettres et chiffres, underscore (_), minus (-), crochets ( [ et ] ), parentheses et slahes ( \ et / )
Tous les autres caracteres sont ignores.
...
L'interface d'admin permet de renommer les smilies. Elle permet aussi de les effacer, ou de les cacher (ils sont toujours dans l'admin, mais non utilisables)
    ...
    1 - Creer un fichier vide smiley.inc.php dans /punbb/ avec un chmod 777 (= autoriser les droits d'ecriture)
    ...
    les lignes a commenter (commenter signifiant mettre // au debut de la ligne) sont les lignes commencant par
		...
		<div class="box">
				<div class="inform">
					<fieldset>
						...
						<div class="infldset">
						<table summary="url smilies" class="aligntop" cellspacing="0">
													<tr>
								<th scope="row">/img/smilies</th>
								<td><div style="height:300px;overflow:auto">
<table summary="smilies list" cellpadding="0" cellspacing="0"><tr>
<?php
...
{
	// Elimination des fichier ., .. et noms vides eventuels
...
while(list($key, $nomfic) = each($tabfile)) 
{
	...
	echo "\n".'<td style="width:'.intval(100/$li).'%;"><div align="center" style="overflow:hidden"><img alt="$nomfic" src="img/smilies/'.$nomfic.'" /><br />';
	...
	if(!$nul) {
		echo '<a name="md5'.$goback.'" id="md5'.$goback.'" style="color:'.$couleur.'" href="javascript:renommer(\''.$nomfic.'\',\''.$code.'\',\''.$goback.'\')">'.$code.'</a> <br />';
