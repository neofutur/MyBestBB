...
<div id="users1" class="blocktable">
	...
	<div class="box">
		<div class="inbox">
		<table summary="liste utilisateurs" cellspacing="0">
		<thead>
			<tr>
...
// Grab the users
$result = $db->query('SELECT u.id, u.group_id, u.username, u.title, u.num_posts, u.registered, g.g_id, g.g_user_title FROM '.$db->prefix.'users AS u LEFT JOIN '.$db->prefix.'groups AS g ON g.g_id=u.group_id WHERE u.id>1'.(!empty($where_sql) ? ' AND '.implode(' AND ', $where_sql) : '').' ORDER BY '.$sort_by.' '.$sort_dir.', u.id ASC LIMIT '.$start_from.', 50') or error('Unable to fetch user list', __FILE__, __LINE__, $db->error());
if ($db->num_rows($result))
{
	while ($user_data = $db->fetch_assoc($result))
	{
		if ( $user_data['group_id'] != 32000 ) 
		{
		 $user_title_field = get_title($user_data);
		}
		else 
		{
		 $user_title_field = "Unverified";
		}
