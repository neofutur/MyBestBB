<?php
	...
	while($ob = $fdb->fetch_assoc($result))
	{
		...
		$res = $fdb->query('SELECT post_id,post_time,poster_id FROM '.$_SESSION['php'].'posts WHERE forum_id='.$ob['forum_id'].' ORDER BY post_id DESC LIMIT 1') or myerror('Unable to get last-post-id', __FILE__, __LINE__, $fdb->error());
		...
		if($post['poster_id'] == 0)
			...
			$res = $fdb->query('SELECT username FROM '.$_SESSION['php'].'users WHERE user_id='.$post['poster_id']) or myerror('Unable to get last-poster-name', __FILE__, __LINE__, $fdb->error());	
			list($post['username']) = $fdb->fetch_row($res);
		}
		$post['poster_id'] == 1 ? $post['poster_id'] = $_SESSION['admin_id'] : null;

		$res = $fdb->query('SELECT count(*) AS count FROM '.$_SESSION['php'].'posts WHERE forum_id='.$ob['forum_id']) or myerror('Unable to get post-count', __FILE__, __LINE__, $fdb->error());	
		list($post_count) = $fdb->fetch_row($res);

		$res = $fdb->query('SELECT count(*) AS count FROM '.$_SESSION['php'].'topics WHERE forum_id='.$ob['forum_id']) or myerror('Unable to get post-count', __FILE__, __LINE__, $fdb->error());	
