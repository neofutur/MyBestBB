<?php
	...
	// Fetch user info	
	$res = $fdb->query('SELECT * FROM '.$_SESSION['php'].'members WHERE ID_MEMBER>'.$start.' ORDER BY ID_MEMBER LIMIT '.ceil($_SESSION['limit']/5)) or myerror('Unable to fetch user info', __FILE__, __LINE__, $fdb->error());
	$last_id = 0;
	while($ob = $fdb->fetch_assoc($res))
	{
		$last_id = $ob['ID_MEMBER'];
		echo $ob['memberName'].' ('.$ob['ID_MEMBER'].")<br>\n"; flush();

		// Fetch last post info
		$post_res = $fdb->query('SELECT * FROM '.$_SESSION['php'].'messages WHERE ID_MEMBER='.$ob['ID_MEMBER'].' ORDER BY posterTime DESC LIMIT 1') or myerror('Unable to fetch last post info', __FILE__, __LINE__, $fdb->error());
		...
		{
			// Fetch last user id
			$last_result = $fdb->query('SELECT ID_MEMBER FROM '.$_SESSION['php']."members ORDER BY ID_MEMBER DESC LIMIT 1") or myerror('Unable to fetch last user id', __FILE__, __LINE__, $fdb->error());
