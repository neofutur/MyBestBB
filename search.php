...
if (isset($_GET['action']) || isset($_GET['search_id']))
{
	...
	if (isset($search_id))
	{
		...
		$query = 'SELECT search_data FROM '.$db->prefix.'search_cache WHERE id='.$search_id.' AND ident=\''.$db->escape($ident).'\'';
		$result = $db->query($query) or error('Unable to fetch search results', __FILE__, __LINE__, $db->error());
	...
	else
	{
		...
		else if ($action == 'show_new' || $action == 'show_24h' || $action == 'show_user' || $action == 'show_subscriptions' || $action == 'show_unanswered')
		{
			...
			if ($action == 'show_new')
			{
				...
				if (!$num_hits)
                                {
                                 // no hits but we still want the chatbox 
                                 if ( $action == 'show_new' && $chatbox == 1 )
                                 {
                                  // chatbox will be added later
                                 }
                                 else message($lang_search['No new posts']);
                                }
			}
...
<div id="vf" class="blocktable">
	...
	<div class="box">
		<div class="inbox">
			<table summary="new posts" cellspacing="0">
			<thead>
<?php  if ( $action == 'show_new' && $chatbox == 1 )
       {
         // adding chatbox
         echo "<tr><td colspan = 4> ";
         require PUN_ROOT.'include/chatbox.php';
         echo "</td></tr> ";
       }
...
<?php
	...
	else
        {
         ...
         else message("4 ".$lang_search['No hits']);
        }
}
...
<?php
         if ( $action == 'show_new' && $chatbox == 1 )
         {
          echo "<table summary='include chatbox'><tr><td colspan = 4> ";
          require PUN_ROOT.'include/chatbox.php';
          echo "</td></tr></table> ";
         }
?>
