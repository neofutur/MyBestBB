<?php
	...
	// Fetch forum info
	$result = $fdb->query('SELECT * FROM '.$_SESSION['php'].'boards') or myerror('Unable to fetch forum info', __FILE__, __LINE__, $fdb->error());

	while($ob = $fdb->fetch_assoc($result)) {

		echo $ob['name'].' ('.$ob['ID_BOARD'].")<br>\n"; flush();

		// Fetch last post info
		$post_res = $fdb->query('SELECT m.posterTime,u.memberName FROM '.$_SESSION['php'].'messages AS m,'.$_SESSION['php'].'members AS u WHERE u.ID_MEMBER=m.ID_MEMBER AND ID_MSG='.$ob['ID_LAST_MSG']) or myerror('Unable to fetch last post info', __FILE__, __LINE__, $fdb->error());
