<?php
	...
	// Fetch topic info
	$result = $fdb->query('SELECT t.*,m.subject,m.posterTime,m.posterName FROM '.$_SESSION['php'].'topics AS t,'.$_SESSION['php'].'messages AS m WHERE t.ID_FIRST_MSG=m.ID_MSG AND t.ID_TOPIC > '.$start.' ORDER BY t.ID_TOPIC LIMIT '.$_SESSION['limit']) or myerror('Unable to get topic info', __FILE__, __LINE__, $fdb->error());
	$last_id = -1;
	while($ob = $fdb->fetch_assoc($result))
	{
		$last_id = $ob['ID_TOPIC'];
		echo '<br>'.$ob['subject'].' ('.$ob['ID_TOPIC'].")\n"; flush();

		// Fetch last post info
		$lastres = $fdb->query('SELECT posterName,posterTime FROM '.$_SESSION['php'].'messages WHERE ID_MSG='.$ob['ID_LAST_MSG'].' LIMIT 1') or myerror('Unable to get last-post-id', __FILE__, __LINE__, $fdb->error());
