...
// add the list bbcode tag , see http://www.punres.org/files/projects/pr_185/rid_321/List%20BBCode%20Readme.txt
//
...
function parse_lists($string) 
{
 ...
 $list = str_replace('[list]', '<br /> <ul>', $list);
 $list = str_replace('[/list]', '</ul> <br />', $list);
 $list = str_replace('<ul><br />', '<ul>', $list );
 $list = str_replace('<ol><br />', '<ol>', $list );
 $list = str_replace('</li><br />', '</li>', $list );
 $list = str_replace('<br /><li', '<li', $list );

 $new_str = str_replace('[list1]', '<br /> <ol>', $list);
 $string = str_replace('[/list1]', '</ol>', $new_str);
...
function do_bbcode($text)
{
        ...
        {
                $text = str_replace('[spoiler]', '</p><blockquote><div class="spoilerbox"><h4>'.$lang_modern_bbcode['Spoiler'
].':</h4><p>', $text);
...
//
// Mod: BBCode noparse (5 nouvelles lignes suivent)
//
...
//			$text = preg_replace('#\[img\]((ht|f)tps?://)([^\s<"]*?)\.(jpg|jpeg|png|gif)\[/img\]#e', 'handle_img_tag(\'$1$3.$4\')', $text);
	...
	$text = str_replace('<p></p>', '', '<p>'.$text.'</p>');
        ...
        // Mod: BBCode noparse (2 nouvelles lignes suivent)
        $text = str_replace('**-NoParseTagBegin-**', '[', $text);
        $text = str_replace('**-NoParseTagEnd-**', ']', $text);
