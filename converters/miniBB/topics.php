<?php
	// Fetch topic info
	$result = $fdb->query('SELECT * FROM '.$_SESSION['php'].'topics WHERE topic_id > '.$start.' ORDER BY topic_id LIMIT '.$_SESSION['limit']) or myerror('phpBB: Unable to get table: topics', __FILE__, __LINE__, $fdb->error());
	$last_id = -1;
	while($ob = $fdb->fetch_assoc($result))
	{
		...
		if($ob['topic_last_post_id'] != 0){
			$lastresult = $db->query('SELECT poster,posted FROM '.$_SESSION['pun'].'posts WHERE id='.$ob['topic_last_post_id']) or myerror("Unable to get user indo", __FILE__, __LINE__, $db->error());
		...
		$res = $fdb->query('SELECT count(*) AS count FROM '.$_SESSION['php'].'posts WHERE topic_id='.$ob['topic_id']) or myerror('Unable to get post-count', __FILE__, __LINE__, $fdb->error());
