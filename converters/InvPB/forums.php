<?php
	...
	$result = $fdb->query(
		'SELECT f.*'
		.' FROM '.$_SESSION['php'].'forums AS f'
		.' WHERE f.id>'.$start
		.' ORDER BY f.id'
		.' LIMIT '.$_SESSION['limit']
	) or myerror('Unable to fetch forum info', __FILE__, __LINE__, $fdb->error());
	$last_id = -1;
	while($ob = $fdb->fetch_assoc($result))
	{
		...
		else
		{
			...
			// Fetch last_post_id
			$res = $fdb->query('SELECT p.pid FROM '.$_SESSION['php'].'posts AS p, '.$_SESSION['php'].'topics AS t WHERE p.topic_id=t.tid AND t.forum_id='.$ob['id'].' ORDER BY p.pid DESC LIMIT 1') or error('Unable to fetch last_post_id', __FILE__, __LINE__, $fdb->error());
