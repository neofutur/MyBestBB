<?php
	...
	// Fetch topic info
	$result = $fdb->query('SELECT * FROM '.$_SESSION['php'].'topics WHERE tid > '.$start.' ORDER BY tid LIMIT '.$_SESSION['limit']) or myerror('Unable to get topic info', __FILE__, __LINE__, $fdb->error());
	$last_id = -1;
	while($ob = $fdb->fetch_assoc($result))
	{
		$last_id = $ob['tid'];
		echo $ob['title'].' ('.$ob['tid'].")<br>\n"; flush();
		...
		// Fetch last_post_id
		$res = $fdb->query('SELECT pid FROM '.$_SESSION['php'].'posts WHERE topic_id='.$ob['tid'].' ORDER BY pid DESC LIMIT 1') or myerror('Unable to get last_post_id', __FILE__, __LINE__, $fdb->error());
