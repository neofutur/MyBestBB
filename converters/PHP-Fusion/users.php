<?php
	...
	{
		$last_id = $ob['user_id'];
		echo $ob['user_name'].' ('.$ob['user_id'].")<br>\n"; flush();

		// Fetch last post info
		$post_res = $fdb->query('SELECT * FROM '.$_SESSION['php'].'posts WHERE post_author='.$ob['user_id'].' ORDER BY post_datestamp DESC LIMIT 1') or myerror('Unable to fetch last post info', __FILE__, __LINE__, $fdb->error());
